...
import { Wind, Hand, Zap, Crosshair, Siren, Shield, Eye, Activity, Flame, Wifi, Map as MapIcon, Maximize2, Minimize2, Radio, Lock } from 'lucide-react';
...
const ActionButton: React.FC<ActionButtonProps> = ({ icon: Icon, label, onClick, color = 'bg-white', textColor = 'text-black', cooldown = 0, maxCooldown = 1, className = '', disabled = false }) => {
    ...
    return (
        <button 
            ...
            className={`group relative w-20 h-20 rounded-full border-4 border-black ${color} flex flex-col items-center justify-center shadow-xl active:scale-95 transition-transform overflow-hidden ${disabled ? 'opacity-50 cursor-not-allowed grayscale' : ''} ${className}`}
        >
            <Icon size={28} className={`${textColor} group-hover:scale-110 transition-transform z-10 ${cooldown > 0 ? 'opacity-50' : ''}`}/>
            <span className={`text-[10px] font-bold ${textColor} z-10 mt-1 ${cooldown > 0 ? 'opacity-50' : ''}`}>{label}</span>
...
const MiniMap = ({ mapObjects, playerPos, theme, isExpanded, onToggle, tasks, sabotageMode, onTrigger, cooldown, systemState }: any) => {
    // Calculate approximate bounds (0 to 2400 usually for this map)
    const WORLD_SIZE = 2400; 
    
    // Filter static geometry for performance
    const mapGeometry = useMemo(() => {
        return mapObjects.filter((o: MapObject) => o.type === 'floor' || o.type === 'wall' || o.type === 'door');
    }, [mapObjects]);

    // Systems locations (Hardcoded based on Zenith map or derived)
    const systems = [
    ...
    return (
        <div 
            ...
            className={`fixed transition-all duration-300 bg-space-900 border-4 border-gray-600 shadow-2xl overflow-hidden z-[100] group ${
                ...
                ? 'inset-12 w-auto h-auto rounded-xl bg-black/90 cursor-default' 
                : 'top-4 right-4 w-48 h-48 rounded-full hover:border-neon-cyan cursor-pointer'
            }`}
        >
            ...
            {sabotageMode && <div className="absolute top-4 left-4 z-50 text-white font-game text-2xl">SABOTAGE SYSTEMS {cooldown > 0 && <span className="text-red-500">({Math.ceil(cooldown)}s)</span>}</div>}

            {/* Map Container */}
            <div className="relative w-full h-full">
                {/* We map 0..WORLD_SIZE to 0..100% */}
...
export const HUD = ({ state, player, onInteract, onAction, onToggleSabotageMap, showSabotageMap, dispatch, canReport }: HUDProps) => {
        ...
        <div className="absolute inset-0 pointer-events-none z-40 p-4 flex flex-col justify-between select-none">
            ...
            <div className="pointer-events-auto flex justify-between items-start">
                ...
                {(!state.systems.comms.active && !isImpostor) ? (
                    <div className="bg-red-900/80 border-2 border-red-500 p-4 rounded-xl shadow-lg w-64 animate-pulse">
                        <div className="text-white font-game text-xl flex items-center gap-2"><Radio className="animate-spin"/> COMMS SABOTAGED</div>
                    </div>
                ) : (
                    <div className="bg-black/60 backdrop-blur-md border-2 border-gray-600 p-3 rounded-xl shadow-lg w-64">
            ...
            {/* Critical Sabotage Warning */}
            {state.activeSabotage && (
                <div className="absolute top-24 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 pointer-events-auto">
                    <div className="bg-red-600/90 text-white px-8 py-3 rounded-full animate-pulse font-game text-xl border-4 border-red-900 flex items-center gap-3 shadow-[0_0_30px_rgba(220,38,38,0.6)]">
                    ...
                    {/* Direction Arrow */}
                    <div className="text-red-500 font-bold animate-bounce text-2xl drop-shadow-md">
            ...
            {/* Action Buttons Grid */}
            <div className="pointer-events-auto flex justify-end items-end gap-4 mb-4 mr-4">
               {role.abilityName && (
